    ...
    try {
      ...
      if (response.success) {
        ...
        const errorMsg =
          typeof response.error === 'string'
            ? response.error
            : errObj?.message || 'Failed to submit lead.';
        setServerError(errorMsg);

        // Handle field-level errors returned by Zod from backend
        if (errObj?.fields) {
          Object.entries(errObj.fields).forEach(([field, msg]) => {
            setError(field as keyof CreateLeadInput, { message: msg });
          });
        }
      }
    } catch (err: unknown) {
      ...
      const message =
        typeof errRes === 'string'
          ? errRes
          : errRes?.message || 'A network or server error occurred. Please try again.';
